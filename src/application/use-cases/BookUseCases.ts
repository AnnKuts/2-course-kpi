...
export class GetBookByIdUseCase {
  constructor(private readonly bookRepository: BookRepository) {}
  public execute = async (id: number): Promise<Book> => {
...
export class CreateBookUseCase {
  ...
  public execute = async (data: CreateBookCommand): Promise<Book> => {
...
export class DeleteBookUseCase {
  constructor(private readonly bookRepository: BookRepository) {}
  public execute = async (id: number): Promise<void> => {
...
export class RateBookUseCase {
  constructor(private readonly bookRepository: BookRepository) {}
  public execute = async (id: number, rating: number): Promise<Book> => {
    const book = await this.bookRepository.findById(id);
    if (!book) throw new NotFoundError(`Book with ID ${id} not found`);
    book.rating = rating;
